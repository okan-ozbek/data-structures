...
- [Key Concepts](#key-concepts)
  ...
  - [Placement New: `new (&data[i]) TValueType(value)`](#placement-new-new-datai-tvaluetypevalue)
  - [Explicit Destructor Calls: `data[i].~TValueType()`](#explicit-destructor-calls-datai-tvaluetype)
  - [Rvalue References: `TValueType&&`](#rvalue-references-tvaluetype)
  - [`std::move`](#stdmove)
...
This `Vector<TValueType>` separates **memory allocation** from **object construction**. This is the same approach that `std::vector` uses internally, and it gives full control over when and where objects are created, moved, and destroyed.

The two memory models in C++:

| High-Level (`new TValueType[]` / `delete[]`)   | Low-Level (`::operator new` / `::operator delete`) |
|------------------------------------------------|-----------------------------------------------------|
...
```cpp
// Allocate raw memory for 10 TValueType objects (no objects are created)
TValueType* data = static_cast<TValueType*>(::operator new(10 * sizeof(TValueType)));

// Free the raw memory (no destructors are called)
::operator delete(data, 10 * sizeof(TValueType));
```

**Why not `new TValueType[10]`?**

`new TValueType[10]` allocates memory **and** default-constructs all 10 elements immediately. For a dynamic array with `size = 0` and `capacity = 10`, you don't want 10 constructed objects sitting in memory — you want empty slots ready for future use.

**Think of it like this:**
- `::operator new` → reserving an empty parking lot (just space)
- `new TValueType[10]` → reserving a parking lot and filling every spot with a car

---

### Placement New: `new (&data[i]) TValueType(value)`
...
```cpp
// Construct a TValueType at the address &data[size_] by copying 'value'
new (&data_[size_]) TValueType(value);

// Construct a TValueType at the address &data[size_] by moving 'value'
new (&data_[size_]) TValueType(std::move(value));

// Construct a TValueType at the address &data[size_] using default constructor
new (&data_[i]) TValueType{};
```
...
| `TValueType(value)`      | Call TValueType's constructor with `value` as argument      |
...
### Explicit Destructor Calls: `data[i].~TValueType()`

When using raw memory, you must **manually destroy objects** before freeing the memory. Calling `~TValueType()` directly runs the destructor without freeing memory.
...
| Operation                          | Effect                          |
|------------------------------------|---------------------------------|
| `new (&data[i]) TValueType(v)`    | Construct an object at address  |
| `data[i].~TValueType()`           | Destroy the object at address   |

**Important:** You should never call `~TValueType()` on an object that was allocated with `new TValueType[]`, because `delete[]` will call the destructor again — resulting in a **double-free** (undefined behavior).

---

### Rvalue References: `TValueType&&`

An **rvalue reference** (`TValueType&&`) binds to **temporary objects** — values that are about to be destroyed and whose resources can be safely stolen.
...
arr.push_back(name);              // Calls push_back(const TValueType&) — 'name' is an lvalue
arr.push_back("world");           // Calls push_back(TValueType&&) — temporary string is an rvalue
arr.push_back(std::move(name));   // Calls push_back(TValueType&&) — std::move casts 'name' to rvalue
```

**Why does this matter?**

- **Copying** (`const TValueType&`): Duplicates all data — allocations, deep copies, etc. Slow for large objects.
- **Moving** (`TValueType&&`): Steals the internal resources (pointers, buffers) from the source. The source is left in a valid but empty state. Fast.
...
```cpp
TValueType item = std::move(data_[size_]);
```

**What happens here:**

1. `std::move(data_[size_])` casts `data_[size_]` to `TValueType&&` (rvalue reference)
2. `TValueType item = ...` calls TValueType's **move constructor** (`TValueType(TValueType&&)`)
...
```cpp
TValueType item = data_[size_];  // Calls COPY constructor — slower, duplicates data
```

**With `std::move`:**

```cpp
TValueType item = std::move(data_[size_]);  // Calls MOVE constructor — faster, steals resources
```

Used in `resize()` to efficiently relocate elements from old memory to new memory:

```cpp
new (&data[i]) TValueType(std::move(data_[i]));  // Move-construct into new buffer
data_[i].~TValueType();                           // Destroy the moved-from husk in old buffer
```
...
void emplace_back(Args&&... args) {
    new (&data_[size_]) TValueType(std::forward<Args>(args)...);
}
```
...
| `std::forward<TValueType>(arg)` | Preserves original category (moves rvalues, copies lvalues) |

`std::forward` ensures that whatever the caller passed in gets forwarded to `TValueType`'s constructor **exactly as it was** — no unnecessary copies, no accidental moves.
...
Think of an iterator as a **generalized pointer**. A raw pointer `TValueType*` already lets you:
...
#### How `VectorIterator` works

The `VectorIterator<TVector>` class is templated on the container type. It stores a single `ValueType*` pointer internally:
...
Step 1: Destroy element at index 1          → data_[1].~TValueType()
Step 2: Move data_[2] into data_[1]         → new (&data_[1]) TValueType(std::move(data_[2]))
Step 3: Move data_[3] into data_[2]         → new (&data_[2]) TValueType(std::move(data_[3]))
Step 4: Move data_[4] into data_[3]         → new (&data_[3]) TValueType(std::move(data_[4]))
Step 5: --size_
...
The full implementation lives in a single header file: [`include/Vector.h`](include/Vector.h)
...
resize(new_capacity):
    ...
    │ 3. Destroy each element in old buffer (~TValueType())        │
...
clear():
    ┌──────────────────────────────────────────────────────────────┐
    │ 1. Call ~TValueType() on each element (indices 0..size_)     │
    │ 2. Set size_ = 0                                             │
...
int main() {
    ...
    arr[0] = 99;                       // Assignment via TValueType& operator[]
